...
simJL = function(n, biasS, biasY) {
  ...
  ### 1. Naive ###
  # note: not Plug-In
  IS0EYW <- apply(WS0, 1, y_dens, n=1000, bias = biasY)
  ...
  ### 2. IPTW ###
  pS1W <- apply(W, 1, s_dens,n=1000, bias=T)
  IS1 <- data$S == 1

  pS0W <- 1 - pS1W
  pS0 <- mean(data$S==0)

  iptw_psi <- mean(IS1/pS1W * pS0W/pS0 * data$Y)
  iptw_psi
  ...
  # define likelihood
  g_dens <- function(task) apply(task$get_node("covariates"),
                                 1, s_dens,n, bias=biasS)
  Q_mean <- function(task) apply(task$get_node("covariates"),
                                 1, y_dens, n, bias=biasY)
  ...
  return(list(tmle = c(tmle_psi, tmle_CI95),
              iptw = c(iptw_psi, iptw_CI95),
              naive_psi = naive_psi))
}
