...
seed_fit = function(seed) {
  ...
  psi <- mean(IS1/prob_clip(pS1W) * pS0W/prob_clip(pS0) * Y)
  # delta method:
  #TODO: IPTW delta method
  #se <- ???
  # by EIC:
  se <- sd(IS1/prob_clip(pS1W) * pS0W/prob_clip(pS0) * (Y - iptw_psi))
...
dat_coverage <- data.frame(Method=c("IPTW", "TML"), 
