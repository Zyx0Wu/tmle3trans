...
#' @param tmle_task a \code{\link{tmle3_Task}} as constructed via \code{point_tr_task}
#' @param learner_list a list of sl3 learners, one for A and one for Y to be used for likelihood estimation
#' @param ... extra arguments.
#' @export
#' @rdname point_tr
point_tr_npsem <- function(node_list, variable_types = NULL) {
  npsem <- list(
...
#' @export
#' @rdname point_tr
point_tr_task <- function(data, node_list, variable_types = NULL, ...) {
  setDT(data)
  
  npsem <- point_tr_npsem(node_list, variable_types)
...
#' @export
#' @rdname point_tr
point_tr_likelihood <- function(tmle_task, learner_list) {
  # covariates
