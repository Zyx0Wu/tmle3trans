Param_TR_old <- R6Class(
  classname = "Param_TR",
  portable = TRUE,
  class = TRUE,
  inherit = Param_base,
  public = list(
    initialize = function(observed_likelihood, initial_likelihood_all,
                          onsite = 1, offsite = 0,
                          ...,
                          covariate_node = "W", site_node = "A", outcome_node = "Y") {
      super$initialize(observed_likelihood, ..., outcome_node = outcome_node)
      private$.initial_likelihood_all <- initial_likelihood_all
      private$.onsite <- onsite
      private$.offsite <- offsite
      private$.covariate_node <- covariate_node
      private$.site_node <- site_node
      private$.cf_likelihood_onsite <- make_CF_Likelihood(initial_likelihood_all, define_lf(LF_static, self$site_node, value = self$onsite))
      private$.cf_likelihood_offsite <- make_CF_Likelihood(initial_likelihood_all, define_lf(LF_static, self$site_node, value = self$offsite))
    },
    clever_covariates = function(tmle_task = NULL, fold_number = "full") {
      training_task <- self$initial_likelihood_all$training_task
      if (is.null(tmle_task)) {
        tmle_task <- self$initial_likelihood_all$training_task
      }

      I1 <- self$cf_likelihood_onsite$get_likelihoods(tmle_task, self$site_node, fold_number)
      p0 <- mean(self$initial_likelihood_all$get_likelihood(training_task, self$site_node, fold_number))
      ...
      p1W <- self$initial_likelihood_all$get_likelihood(cf_task_onsite, self$site_node, fold_number)
      p0W <- self$initial_likelihood_all$get_likelihood(cf_task_offsite, self$site_node, fold_number)
